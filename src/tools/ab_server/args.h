...
/*
 * Copied from src/poc/utils/args.[ch].  That version returns the poc tree's util_err_t from
 * err.h; ab_server has its own unrelated err.h, so the handful of codes this module actually
 * uses are declared here instead of dragging in a second error enum.  Values are the ARGS_*
 * subset -- everything else in the original enum was for other modules.
 */
typedef enum {
    UTIL_OK = 0,
    UTIL_EINVAL,
    UTIL_ERESOURCE,
    UTIL_EARGS_INVALID_FORMAT,
    UTIL_EARGS_UNKNOWN_FLAG,
    UTIL_EARGS_DUPLICATE,
    UTIL_EARGS_INVALID_VALUE,
    UTIL_EARGS_MISSING_REQUIRED,
} util_err_t;
