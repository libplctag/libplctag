...
{
    int32_t tagHandles[2] = {
        plc_tag_create(tagPath1, DATA_TIMEOUT),
        plc_tag_create(tagPath2, DATA_TIMEOUT)
    };

    for (int i = 0; i < 2; i++)
        if (tagHandles[i] < 0)
            FATAL("Error[%d] creating tag handle: %s\n", i, plc_tag_decode_error(tagHandles[i]));
    ...
    for (int t = 0; t < TRY_TIMES; t++) {
        ...
        util_sleep_ms(TIME_BETW_OP);
    }

    for (int i = 0; i < 2; i++)
        plc_tag_destroy(tagHandles[i]);
    return;
}
