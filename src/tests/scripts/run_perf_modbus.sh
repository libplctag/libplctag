...
#
#   --executable-dir=DIR  directory containing perf_benchmark and modbus_server binaries
...
MODBUS_SERVER="${EXECUTABLE_DIR}/modbus_server"
...
kill_modbus_server() {
    # Kill any running modbus_server instances (including ones not started by this script).
...
start_modbus_server() {
    ...
    echo "Starting modbus_server on port $MODBUS_SERVER_PORT..." >&2
    "$MODBUS_SERVER" \
    ...
    if ! kill -0 "$MODBUS_SERVER_PID" 2>/dev/null; then
        echo "ERROR: modbus_server failed to start." >&2
        exit 1
    fi
    echo "modbus_server running (PID $MODBUS_SERVER_PID)." >&2
}
    ...
    for threads in "${STEPS[@]}"; do
        ...
        for mode in "${MODES[@]}"; do
            ...
            if csv_line=$("$PERF_BENCHMARK" \
                ...
                # Restart modbus_server in case it crashed.
                if [ -n "$MODBUS_SERVER_PID" ] && ! kill -0 "$MODBUS_SERVER_PID" 2>/dev/null; then
                    echo "  -> modbus_server died, restarting..." >&2
                    start_modbus_server
                fi
            fi
        done
