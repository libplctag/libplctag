...
# Only require ab_server when running in simulator mode (no --tag supplied).
...
kill_ab_server() {
    # Kill any running ab_server instances (including ones not started by this script).
    ...
    for threads in "${STEPS[@]}"; do
        ...
        for mode in "${MODES[@]}"; do
            ...
            if csv_line=$("$PERF_BENCHMARK" \
                ...
                # Restart ab_server in case it crashed (simulator mode only).
