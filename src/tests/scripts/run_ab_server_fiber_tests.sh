...
{ $TEST_DIR/ab_server_fiber --debug=on --plc=ControlLogix --path=1,0 "--tag=TestBigArray:DINT[2000]" "--tag=Test_Array_1:DINT[1000]" "--tag=Test_Array_2x3:DINT[2,3]" "--tag=Test_Array_2x3x4:DINT[2,3,4]" > "$LOG_DIR/logix_fast_emulator.log" 2>&1 & } 2>/dev/null
EMULATOR_PID=$!
...
let TEST++
echo -n "  Test $TEST: EtherNet/IP ListIdentity returns the neutral CIP identity... "
$VALGRIND$TEST_DIR/test_list_identity 127.0.0.1 44818 > "$LOG_DIR/${TEST}_list_identity_test.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE"
    let FAILURES++
else
...
{ $VALGRIND$TEST_DIR/ab_server_fiber --plc=ControlLogix --path=1,0 "--tag=TestBigArray:DINT[2000]" "--tag=Test_Array_1:DINT[1000]" "--tag=Test_Array_2x3:DINT[2,3]" "--tag=Test_Array_2x3x4:DINT[2,3,4]" --delay=100  > "$LOG_DIR/logix_slow_emulator.log" 2>&1 & } 2>/dev/null
EMULATOR_PID=$!
if [ $EMULATOR_PID -le 0 ]; then
    echo "Unable to start AB/ControlLogix emulator!"
    exit 1
...
{ $TEST_DIR/ab_server_fiber --debug=on --plc=Micro800 --tag=TestDINTArray:DINT[10] > "$LOG_DIR/micro800_emulator.log" 2>&1 & } 2>/dev/null
EMULATOR_PID=$!
if [ $EMULATOR_PID -le 0 ]; then
    # echo "FAILURE"
...
{ $TEST_DIR/ab_server_fiber --debug=on --plc=Omron --tag=TestDINTArray:DINT[10] > "$LOG_DIR/omron_emulator.log" 2>&1 & } 2>/dev/null
EMULATOR_PID=$!
if [ $EMULATOR_PID -le 0 ]; then
    # echo "FAILURE"
...
{ $TEST_DIR/ab_server_fiber --debug=on --plc=Micrologix '--tag=B3[10]' '--tag=N7[10]' '--tag=L19[10]' > "$LOG_DIR/micrologix_emulator.log" 2>&1 & } 2>/dev/null
EMULATOR_PID=$!
if [ $EMULATOR_PID -le 0 ]; then
    # echo "FAILURE"
...
{ $TEST_DIR/ab_server_fiber --debug=on --plc=PLC/5 '--tag=B3[10]' '--tag=N7[10]' > "$LOG_DIR/plc5_emulator.log" 2>&1 & } 2>/dev/null
EMULATOR_PID=$!
if [ $EMULATOR_PID -le 0 ]; then
    # echo "FAILURE"
