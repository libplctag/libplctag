...
pkill -TERM modbus_server_coro > /dev/null 2>&1

# wait for them to exit
sleep 2

# echo -n "  Starting Modbus server $SCRIPT_DIR/modbus_server_coro... "
$TEST_DIR/modbus_server_coro --listen=127.0.0.1:1502 --listen=127.0.0.1:2502 --debug=DETAIL > modbus_server_coro.log 2>&1 &
MODBUS_PID=$!
if [ $? != 0 ]; then
    # echo "FAILURE"
