...
# Section timing: a "section" is the span of tests run against one emulator/server
# lifetime, delineated by the next "Starting ... emulator/server" line. SCRIPT_START
# covers the whole run; SECTION_START is reset at each such boundary (see below).
SCRIPT_START=$(date +%s)
...
if [ $? != 0 ]; then
    echo "FAILURE ($(( $(date +%s) - TEST_START ))s)"
    let FAILURES++
else
    echo "OK ($(( $(date +%s) - TEST_START ))s)"
    let SUCCESSES++
fi
...
$VALGRIND$TEST_DIR/test_connection_tag "--tag=protocol=ab-eip&gateway=127.0.0.1&path=1,0&plc=ControlLogix&connection_inactivity_timeout_ms=5000&name=@connection" > "$LOG_DIR/${TEST}_connection_tag_test.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE ($(( $(date +%s) - TEST_START ))s)"
    let FAILURES++
else
...
echo "  [Section time: $(( $(date +%s) - SECTION_START ))s]"
SECTION_START=$(date +%s)
...
# These two run concurrently, each against its own ab_server on its own port
# (test_reconnect_after_outage_async/_sync each start and kill their own server,
# tracked by a port-specific pidfile so they don't collide). Wall time for this
# pair is max(async, sync) instead of async + sync.
let TEST++
TEST_ASYNC_NUM=$TEST
TEST_ASYNC_START=$(date +%s)
echo -n "  Test $TEST: Test async reconnect after PLC outage... "
$VALGRIND$TEST_DIR/test_reconnect_after_outage_async "${TEST_DIR}/ab_server" 44819 > "$LOG_DIR/${TEST}_reconnect_after_outage_async.log" 2>&1 &
ASYNC_PID=$!
echo "(started)"

let TEST++
TEST_SYNC_NUM=$TEST
TEST_SYNC_START=$(date +%s)
echo -n "  Test $TEST: Test sync reconnect after PLC outage... "
$VALGRIND$TEST_DIR/test_reconnect_after_outage_sync "${TEST_DIR}/ab_server" 44820 > "$LOG_DIR/${TEST}_reconnect_after_outage_sync.log" 2>&1 &
SYNC_PID=$!
echo "(started, running in parallel with test $TEST_ASYNC_NUM)"

wait $ASYNC_PID
if [ $? != 0 ]; then
    echo "  Test $TEST_ASYNC_NUM result: FAILURE ($(( $(date +%s) - TEST_ASYNC_START ))s)"
    let FAILURES++
else
    echo "  Test $TEST_ASYNC_NUM result: OK ($(( $(date +%s) - TEST_ASYNC_START ))s)"
    let SUCCESSES++
fi

wait $SYNC_PID
if [ $? != 0 ]; then
    echo "  Test $TEST_SYNC_NUM result: FAILURE ($(( $(date +%s) - TEST_SYNC_START ))s)"
    let FAILURES++
else
    echo "  Test $TEST_SYNC_NUM result: OK ($(( $(date +%s) - TEST_SYNC_START ))s)"
    let SUCCESSES++
fi
...
$VALGRIND$TEST_DIR/test_connection_tag "--tag=protocol=ab-eip&gateway=127.0.0.1&path=18,127.0.0.1&plc=omron-njnx&connection_inactivity_timeout_ms=5000&name=@connection" > "$LOG_DIR/${TEST}_connection_tag_omron_test.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE ($(( $(date +%s) - TEST_START ))s)"
    let FAILURES++
else
...
$VALGRIND$TEST_DIR/test_connection_tag "--tag=protocol=modbus-tcp&gateway=127.0.0.1:1502&path=0&connection_inactivity_timeout_ms=5000&name=@connection" > "$LOG_DIR/${TEST}_connection_tag_modbus_test.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE ($(( $(date +%s) - TEST_START ))s)"
    let FAILURES++
else
...
if [ "${PLC_COUNT}" = "2" ] ; then
    echo "OK (found ${PLC_COUNT} PLC creation entries in log file ${TST_LOG}) ($(( $(date +%s) - TEST_START ))s)"
    let SUCCESSES++
else
    echo "FAILURE (expected 2 PLC creation entries, found ${PLC_COUNT} in log file ${TST_LOG}) ($(( $(date +%s) - TEST_START ))s)"
    let FAILURES++
fi
...
echo " - Total time: $(( $(date +%s) - SCRIPT_START ))s"
