...
let TEST++
echo -n "Test $TEST: enip-udp unicast List Identity discovery... "
$VALGRIND$TEST_DIR/discover_identity "--tag=protocol=enip-udp&gateway=10.206.1.40&name=@identity" > "$LOG_DIR/${TEST}_discover_identity_unicast.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE"
    let FAILURES++
else
...
let TEST++
echo -n "Test $TEST: enip-udp broadcast List Identity discovery (10.206.1.0/24)... "
$VALGRIND$TEST_DIR/discover_identity "--tag=protocol=enip-udp&gateway=10.206.1.255/24&name=@identity" > "$LOG_DIR/${TEST}_discover_identity_broadcast.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE"
    let FAILURES++
else
