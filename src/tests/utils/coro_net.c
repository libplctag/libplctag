...
static int socketpair(int domain, int type, int protocol, socket_t pair[2]) {
    ...
    if (socket_address_init(&addr, "127.0.0.1", 0) != UTIL_OK) {
        return -1;
    }
...
util_err_t coro_run(coro_net_t **net_ptr, uint32_t tick_interval_ms) {
    ...
    while (net->running) {
        ...
        for (size_t i = 0; i < net->max_tasks; i++) {
            ...
            if (net->task_fds[actual_index] != INVALID_SOCKET) { /* is this an allocated task? */
                ...
                if (net->task_fds[actual_index] != CORO_NO_SOCKET && net->task_events[actual_index] != CORO_EVENT_NONE) {
                    pdlog(LOG_MODULE_CORO_NET, LOG_LEVEL_SPEW, "Adding socket task to poll array: index=%zu fd=%d events=0x%04x", actual_index, (int)net->task_fds[actual_index], net->task_events[actual_index]);
        ...
        if(net->iteraction_count % 100 == 0) {
            ...
            pdlog(LOG_MODULE_CORO_NET, LOG_LEVEL_DETAIL, "Iteration %zu: rebuild=%" PRId64 "us, poll=%" PRId64 "us, dispatch=%" PRId64 "us, nfds=%zu",
