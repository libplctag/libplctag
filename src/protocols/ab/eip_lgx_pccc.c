...
int eip_lgx_pccc_tag_read_start(ab_tag_p tag)
{
    ...
    embed_pccc->pccc_function = AB_EIP_PCCC_TYPED_READ_FUNC;
    embed_pccc->pccc_transfer_size = h2le16(tag->elem_count); /* This is the offset items */
    ...
    *((uint16_le*)data) = h2le16(tag->elem_count); /* FIXME - bytes or INTs? */
    ...
    lgx_pccc->uc_cmd_length = h2le16(data - embed_start);
    ...
    lgx_pccc->cpf_udi_item_length   = h2le16(data - (uint8_t*)(&lgx_pccc->cm_service_code));
...
static int check_read_status(ab_tag_p tag)
{
    ...
    do {
        ...
        tag->encoded_type_info_size = (type_end - type_start);
    ...
    if (rc == PLCTAG_STATUS_OK && tag->pre_write_read) {
        ...
        rc = eip_lgx_pccc_tag_write_start(tag);
    }
...
//    int pccc_data_type;
    ...
    if (tag->first_read) {
        ...
        return eip_lgx_pccc_tag_read_start(tag);
    }
    ...
    embed_pccc->pccc_status = 0;  /* STS 0 in request */
    embed_pccc->pccc_seq_num = h2le16(conn_seq_id); /* FIXME - get sequence ID from session? */
    embed_pccc->pccc_function = AB_EIP_PCCC_TYPED_WRITE_FUNC;
